...
logger = logging.getLogger("auditforge.connectors.mysql")


class MySQLConnector(BaseConnector):
    """Connect to MySQL databases using *pymysql*."""
    ...
    async def connect(self, target: Any) -> bool:
        ...
        try:
            ...
            logger.error("MySQL connection to %s:%s failed: %s", host, port, exc)
    ...
    async def disconnect(self) -> None:
        if self._conn is not None:
            try:
                ...
                pass
            self._conn = None
            logger.info("MySQL disconnected from %s", self._host)
